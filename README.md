...
1. **Install requirements**: `pip install requests python-dotenv`. Ensure `.env` at the project root contains `GOOGLE_API_KEY=…` (the script automatically loads it via `dotenv.find_dotenv()`), or export the same key before running. Run the CLI from `/Users/lulu/Desktop/GenAI_Ass2` so `find_dotenv` locates `.env`. The script now calls the Gemini REST endpoint (`generateText`) directly with a 15‑second timeout to avoid hanging.
